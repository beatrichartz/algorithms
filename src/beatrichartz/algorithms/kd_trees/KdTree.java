...
public class KdTree implements APointSet {
    private class Node {
    ...
    public Iterable<Point2D> range(RectHV rect) {
        LinkedListNode firstDummy = new LinkedListNode(null);
        range(firstDummy, root, rect, new RectHV(0, 0, 1, 1), 0);
        return new PointsIterable(firstDummy.next);
    }
    ...
    private LinkedListNode range(LinkedListNode points, Node node, RectHV queryRect, RectHV subTreeRect, int level) {
        ...
        if (level % 2 == 0) {
            points = range(points, node.left, queryRect, new RectHV(xmin, ymin, x, ymax), level + 1);
            points = range(points, node.right, queryRect, new RectHV(x, ymin, xmax, ymax), level + 1);
        } else {
            points = range(points, node.left, queryRect, new RectHV(xmin, ymin, xmax, y), level + 1);
            points = range(points, node.right, queryRect, new RectHV(xmin, y, xmax, ymax), level + 1);
        }
